...
for (const viewport of [{ width: 1366, height: 900 }, { width: 390, height: 844 }, { width: 844, height: 390 }]) {
  test(`all nine vault spreads render at ${viewport.width}x${viewport.height}`, async ({ page }, testInfo) => {
    ...
    for (const [index, artifact] of memoryArtifacts.entries()) {
      ...
      // Validate that the final prose is reachable without paying Playwright's expensive
      // actionability/scroll animation cost for every memory on software-rendered CI.
      ...
      // One representative visual artifact per viewport is enough; all nine entries still
      // receive DOM, image, text, overflow and reading-end validation above.
      if (index === 0) {
