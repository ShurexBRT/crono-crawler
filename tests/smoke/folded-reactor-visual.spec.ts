...
test('captures the Folded Reactor production target across all timelines', async ({ page }) => {
  ...
  await page.screenshot({ path: 'artifacts/visual/folded-reactor-present.png', fullPage: true });

  await focusPlayfield(page);
  await page.keyboard.press('1');
  await expect(page.locator('[data-hud="timeline"]')).toHaveText('Past');
  await page.waitForTimeout(240);
  await page.screenshot({ path: 'artifacts/visual/folded-reactor-past.png', fullPage: true });

  await focusPlayfield(page);
  await page.keyboard.press('3');
  await expect(page.locator('[data-hud="timeline"]')).toHaveText('Ruined Future');
  await page.waitForTimeout(240);
  await page.screenshot({ path: 'artifacts/visual/folded-reactor-future.png', fullPage: true });
